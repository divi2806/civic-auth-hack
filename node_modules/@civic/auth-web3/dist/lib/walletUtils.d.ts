import type { CivicWeb3ClientConfig, EVMChainRpcUrls, ExistingWeb3UserContext, NewWeb3UserContext, RpcUrls, Web3UserContextType } from "../types.js";
import { type Chain } from "viem";
export declare const ICON: "data:image/svg+xml;base64,";
export declare const isNewUser: (userContext: Web3UserContextType) => userContext is NewWeb3UserContext;
export declare const userHasWallet: (userContext: Web3UserContextType) => userContext is ExistingWeb3UserContext;
/**
 * Convert a chain to a set of RPC urls by using the defaults
 * @param chain
 */
export declare const defaultRpcNodeUrlsForChain: (chain: Chain) => RpcUrls;
export declare const getEthereumProviderRPCsFromConfig: (config: CivicWeb3ClientConfig) => EVMChainRpcUrls;
//# sourceMappingURL=walletUtils.d.ts.map